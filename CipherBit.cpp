...
void CipherBit::notLogic(){
  Plaintext myPlaintext("1");
  Ciphertext myCipher; 
  (this -> encryptor).encrypt(myPlaintext,myCipher);
  (this -> evaluator) -> sub(myCipher,this->cipherBit);
  (this->cipherBit)=myCipher;
}

CipherBit strictPlusPetit(CipherBit a,CipherBit b){
	CipherBit tmp=a;
	tmp.notLogic();
	(this -> evaluator) -> multiply(b.getcipherBit);
	return tmp;
}
